...
# 第四十一部分：课程节点评估（2026-09-05）

评估背景：

- lesson01 ~ lesson05 全部完成，Contact Manager V5 三层架构定型
- 远程仓库分支已清理（仅剩 master），lesson05 内容已合并推送
- 本部分是"课程 + 项目"的整体快照，供下一阶段教学决策使用

---

## 一、项目代码评估（Contact Manager V5）

### 架构成熟度（已达标）

| 维度 | 状态 |
|---|---|
| 分层分离 | ✅ UI / 业务 / 存储 三层各自独立成模块 |
| 依赖方向 | ✅ ui 不 import contacts / storage；main 依赖 Storage 协议而非实现 |
| 接口契约 | ✅ Storage Protocol + 自定义异常层次 + 异常链 |
| 状态管理 | ✅ is_dirty 快照语义正确（改回原样则不保存） |

### 遗留缺陷清单（按严重度排序）

1. 【中】显示与输入不匹配

   `show_contacts()` 打印列表不带编号，删除流程却要求"请输入编号"。
   自 L1 遗传至今。修复成本：`enumerate` 一行。

   教学价值：UI 契约意识——"展示什么"决定"用户能输入什么"。

2. 【中】损坏检测漏洞

   `load()` 只把 `JSONDecodeError` 视为损坏；合法 JSON 但结构错误
   （如 `null`、`[1,2,3]`、缺 phone 键）会在 `from_data` 抛出
   `KeyError` / `TypeError`，main 未捕获 → 直接崩溃。

   教学价值：数据结构校验应该放在存储层还是业务层。

3. 【低】验证不严

   `validate_name` 只挡 `== ""`（纯空格、None 可通过）；
   `isdigit` 放行全角数字（`"１３８..."`）。

4. 【低】change_phone 不查重

   `add_contact` 与 `from_data` 都查重复号码，唯独 `change_phone` 没有。
   当前没有编辑菜单所以未暴露，一旦加编辑功能即成 bug。

5. 【低】CI 双红灯（工程健康度）

   - test job：`import main` 触发 lesson01 顶层交互循环 →
     无 stdin 环境抛 `EOFError`（结构性缺陷，工作流创建起即失败）
   - lint job：CI 安装 ruff 0.16 默认规则变严，
     lesson05 存在 I001 ×2、PIE790 ×3
   - 不影响代码功能，但削弱"master 随时可发布"的信心

### 范式上升空间（第五阶段后半候选素材）

- `@dataclass` 消灭 Contact 样板（约 180 行手写 → 约 10 行）
- `Enum` + `match` 重构菜单分发（消除魔法字符串）
- 观察者模式替代快照脏标记（O(n) 轮询比对 → O(1) 变更通知）
- 依赖注入 + 可测试性（最关键：让 Protocol 真正兑现价值）
...
```text
L1  能跑        过程式脚本，全局状态，无持久化
L2  会拆模块    函数分层 + dict 数据模型 + JSON，但模块级全局状态
L3  有了对象    Contact / ContactBook 出现，但 print 泄漏进业务类
L4  对象有纪律  私有属性 + property + 构造时验证 + 原子 from_data
L5  项目有架构  Protocol + 异常层次 + UI 成层 + 脏标记 + 入口守卫
```

一句话：**这条路正是真实项目重构的路径，L3 那次 UI 泄漏的倒退也很真实。**

### 能力矩阵

| 档位 | 内容 |
|---|---|
| 已独立掌握 | 封装与私有约定、property、构造时验证、异常层次设计、分层职责划分、Git 分支与合并流程 |
| 已理解待实践 | Protocol 依赖倒置（知道为什么，尚未体会"替换实现"的收益）、Composition 基础 |
| 未接触 | 继承、多态、dataclass、依赖注入、单元测试 |
...
- Protocol 已完成依赖倒置的"形"，注入才让它有"实"
- 能注入 = 能用 FakeStorage 替换文件系统 = 单元测试可行
- 顺带消化遗留缺陷 1、2 作为重构练习（先写测试，后修复）

### 推荐路径

```text
第1课  测试思维：为什么分层之后才可能测试？
第2课  构造器注入：App(storage, ui)
第3课  FakeStorage 跑通全流程，顺带修复缺陷 1、2
第4课  dataclass 与 Enum：用新语法回收旧样板
```
